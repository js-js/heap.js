...
MarkCopy.prototype.run = function run(space) {
  ...
  while (queue.length !== 0) {
    ...
    // Queue all ancestors
    this.heap.visit(dst || edge.to, function(data, off, tag, weak) {
  ...
  weakQueue.forEach(function(edge) {
    if (this.isMarked(edge.space, edge.to, edge.tag))
      ...
      binding.writeTagged(edge.data, this.heap.hole.deref(), edge.offset);
  }, this);

  // Swap marks
  this.heap.spaces.data.mark = (~this.heap.spaces.data.mark) & 1;
  this.heap.spaces.code.mark = (~this.heap.spaces.code.mark) & 1;
...
MarkCopy.prototype.setMark = function setMark(edge, dst) {
  ...
  m = (edge.space.mark << 3) | Math.min((m & 0x7) + 1, 0x7);
