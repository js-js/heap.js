...
Space.prototype.allocRaw = function allocRaw(num) {
  ...
  // Retry after gc
  res = this.currentPage.alloc(num);
  if (res)
    return res;
...
Space.prototype.swapSemi = function swapSemi() {
  ...
  // Swap startRef/limitRef
  var tmp = new Buffer(this.startRef.length);
  semi.startRef.copy(tmp);
  this.startRef.copy(semi.startRef);
  tmp.copy(this.startRef);
  semi.limitRef.copy(tmp);
  this.limitRef.copy(semi.limitRef);
  tmp.copy(this.limitRef);
};
