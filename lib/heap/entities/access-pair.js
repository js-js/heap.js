...
AccessPair.alloc = function alloc(heap, pair) {
  ...
  if (pair.enumerable === false)
    attrs = heap.smi(attrs.value() & ~AccessPair.attributes.enumerable);
...
AccessPair.prototype.attributes = function attributes() {
  return this.heap.wrap(binding.readTagged(this.deref(), offsets.attributes));
};

AccessPair.prototype.updateAttributes = function updateAttributes(handle) {
  binding.writeTagged(this.deref(), handle.deref(), offsets.attributes);
};
