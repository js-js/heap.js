  ...
  if (update) {
    ...
    if (res.index === null) {
      return this.heap.scope(function() {
        this.grow();
        return this._getPropertySlot(key, update);
      }, this);
...
Object.prototype.getPropertySlot = function getPropertySlot(key, update) {
  var res = this._getPropertySlot(key, update);
  if (res === null)
...
Object.prototype.set = function set(key, value, noTransition) {
  var slot = this._getPropertySlot(key, true, noTransition);

  this.field().set(slot * 2 + 1, value);
};

Object.prototype.get = function get(key, shallow) {
  var slot = this._getPropertySlot(key, false);

  if (slot !== null)
    return this.field().get(slot * 2 + 1);
