...
var ArrayDict = heap.entities.ArrayDict;
...
Object.prototype.grow = function grow(min) {
  ...
  for (; ; nsize *= 2) {
    ...
    for (var i = 0; i < size; i++) {
      ...
      ndict.setAttr(slot, dict.getAttr(i));
    }
...
Object.prototype.set = function set(key, value, options) {
  ...
  var attrs = options && options.attributes ||
              this.heap.smi(AttrDict.attributes['default']);

  if (this.isDense()) {
    // Non Smi - transition to regular array
    if (key.type !== 'smi' ||
        (attrs.value() & ~ArrayDict.attributes.mask) !== 0) {
...
Object.prototype.iterate = function iterate(cb) {
  ...
  for (var i = 0; i < size; i++) {
    ...
    var attrs = dict.getAttr(i).cast().value();
    if (attrs & AttrDict.attributes.enumerable) {
      var value = dict.getValue(i);
      cb(key, value);
    }
  }
