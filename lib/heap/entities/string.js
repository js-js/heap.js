...
String.alloc = function alloc(heap, value) {
  var len = Buffer.isBuffer(value) ? value.length : Buffer.byteLength(value);
...
String.prototype.data = function data() {
  return this.deref().slice(offsets.data);
};

String.prototype.value = function value() {
  var ptr = this.deref();
...
String.prototype._add = function _add(other) {
  var len = this.length();
  var otherLen = other.length();
  var result = new Buffer(len + otherLen);

  this.data().copy(result, 0, 0, len);
  other.data().copy(result, len, 0, otherLen);

  return this.heap.allocString(result);
};
