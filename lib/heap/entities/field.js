...
Field.alloc = function alloc(h, size) {
  var res = h.allocTagged('data', 'field', Field.size(size));

  // Field size
  binding.writeTagged(res, size, offsets.size);

  // Field contents (holes)
  var hole = h.hole.deref();
  for (var i = 0; i < size; i++)
    binding.writeTagged(res, hole, offsets.field + i *  heap.ptrSize);

  return new Field(h, res);
};
