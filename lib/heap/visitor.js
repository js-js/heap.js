...
Visitor.prototype.visit = function visit(ptr, cb) {
  ...
  if (handle instanceof heap.entities.Function) {
    this.visitFunction(handle, ptr, cb);
  } else if (handle instanceof heap.entities.Object) {
  ...
  } else if (handle instanceof heap.entities.AccessPair) {
    this.visitAccessPair(handle, ptr, cb);
  }
};
...
Visitor.prototype.visitCode = function visitCode(code, ptr, cb) {
  ...
  code.weakOffsets().forEach(function(off) {
    cb(ptr, Code.offsets.code + off, 'weak-code-slot', true);
  });
};
...
Visitor.prototype.visitFunction = function visitFunction(fn, ptr, cb) {
  ...
  // Object's references
  this.visitObject(fn, ptr, cb);

  // Code's code
  cb(ptr, [ Function.offsets.code, Code.offsets.code ], 'code');

  // Instance map
  cb(ptr, Function.offsets.instanceMap, 'instance-map');
};
...
Visitor.prototype.visitAccessPair = function visitAccessPair(str, ptr, cb) {
  var AccessPair = heap.entities.AccessPair;
  cb(ptr, AccessPair.offsets.getter, 'getter');
  cb(ptr, AccessPair.offsets.setter, 'setter');
};
