...
function Heap(options) {
  ...
  this.global = this.allocGlobal(32);
}
module.exports = Heap;
...
Heap.prototype.scope = function scope(cb, ctx) {
  ...
  if (res instanceof heap.entities.Base)
...
Heap.prototype._allocObject = function _allocObject(map, size) {
  ...
  var res = this.allocTagged('data', map, heap.ptrSize);
...
Heap.prototype.allocObject = function allocObject(size) {
  return this._allocObject('object', size);
};

Heap.prototype.allocGlobal = function allocGlobal(size) {
  return this._allocObject('global', size);
};

Heap.prototype.allocContext = function allocContext() {
  var res = this.allocTagged('data', 'context', 3 * heap.ptrSize);

  binding.writeTagged(res, this.global.deref(), heap.ptrSize);
  binding.writeTagged(res, this.global.deref(), 2 * heap.ptrSize);
  binding.writeTagged(res, this.maps.hole.deref(), 3 * heap.ptrSize);

  return new heap.entities.Context(this, res);
};
...
Heap.prototype.allocBoolean = function allocBoolean(value) {
  var res = this.allocTagged('data', 'boolean', heap.ptrSize);

  binding.writeTagged(res, value ? 1 : 0, heap.ptrSize);

  return new heap.entities.Boolean(this, res);
};
