...
function Heap(options) {
  ...
  this.registerMap('object', 'map', Map.flags.object);
  ...
  this.registerMap('function', 'object', Map.flags.object);
...
Heap.prototype.allocMap = function allocMap(parent, flags) {
  ...
  if (this.maps.object) {
    this.virtualScope(function() {
      ...
      // Hidden-class edges
      var obj = this.allocObject();
      binding.writeTagged(res, obj.deref(), Map.offsets.edges);
    }, this);
...
Heap.prototype._allocObject = function _allocObject(map, size, fieldSize) {
  ...
  if (!fieldSize)
    fieldSize = 0;
  fieldSize = Math.max(heap.entities.HashMap.minSize, fieldSize);
  ...
  this.virtualScope(function() {
    binding.writeTagged(res,
                        this.allocHashMap(fieldSize).deref(),
...
Heap.prototype.registerMap = function registerMap(name, parent, flags) {
  var map = this.allocMap(parent, flags);
  this.maps[name] = map;
  return map;
};
