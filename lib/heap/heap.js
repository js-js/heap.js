...
function Heap(options) {
  ...
  this.registerMap('field', 'map');
  this.registerMap('object', 'map', Map.flags.object | Map.flags.transition);
...
Heap.prototype.scope = function scope(cb, ctx) {
  ...
  var res = scope.wrap(function scopeWrap() {
...
Heap.prototype.allocMap = function allocMap(parent, flags) {
  var Map = heap.entities.Map;
  ...
  binding.writeTagged(res,
                      typeof parent === 'string' ?
                          this.maps[parent].deref() :
                          parent ? parent.deref() : res,
                      Map.offsets.parent);
...
Heap.prototype.allocString = function allocString(val) {
  ...
  binding.writeTagged(res, this.hole.deref(), String.offsets.hash);
...
Heap.prototype.allocField = function allocField(size) {
  ...
  var res = this.allocTagged('data', 'field', Field.size(size));

  // Field size
  binding.writeTagged(res, size, Field.offsets.size);

  // Field contents (holes)
  var hole = this.hole.deref();
  for (var i = 0; i < size; i++) {
    binding.writeTagged(res,
                        hole,
                        Field.offsets.field + i *  heap.ptrSize);
  }

  return new Field(this, res);
};
...
Heap.prototype._allocObject = function _allocObject(map, size) {
  ...
  this.virtualScope(function() {
    binding.writeTagged(res,
                        this.allocField(3 * fieldSize).deref(),
...
Heap.prototype.allocFunction = function allocFunction(code) {
  ...
  var Map = heap.entities.Map;
  var res = this._allocObject('function', Function.size());

  res = new Function(this, res);
  res._setCode(code);

  this.virtualScope(function() {
    var instanceMap = this.allocMap('object',
                                    Map.flags.object |
                                        Map.flags.transition);
...
Heap.prototype.allocContext = function allocContext() {
  ...
  this.virtualScope(function() {
    var global = this.allocGlobal().deref();
...
Heap.prototype.allocCode = function allocCode(buf, offsets, weak) {
  ...
  var size = Code.size(codeSize, offsets.length, weak.length);
  var res = this.allocTagged('code', 'code', size);

  binding.writeTagged(res, codeSize, Code.offsets.size);
  binding.writeTagged(res, offsets.length, Code.offsets.offsetCount);
  binding.writeTagged(res, weak.length, Code.offsets.weakCount);
...
Heap.prototype.allocAccessPair = function allocAccessPair(pair) {
  ...
  var res = this.allocTagged('data', 'access-pair', AccessPair.size());

  if (!pair)
    pair = {};

  binding.writeTagged(res,
                      (pair.getter || this.hole).deref(),
                      AccessPair.offsets.getter);
  binding.writeTagged(res,
                      (pair.setter || this.hole).deref(),
                      AccessPair.offsets.setter);

  return new AccessPair(this, res);
};
...
Heap.prototype.registerMap = function registerMap(name, parent, flags) {
  var map = this.allocMap(parent, flags | 0);
  this.maps[name] = map;
  return map;
};
