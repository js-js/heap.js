...
function Heap() {
  ...
  this.codeOffs = new heap.scopes.Scope(this);
  this.codeOffs.type = 'code-offs';
  this.codeOffs.enter();
...
Heap.prototype.allocCode = function allocCode(buf, offsets) {
  ...
  for (var i = 0; i < offsets.length; i++, off += heap.ptrSize) {
    var realOffset = 2 * heap.ptrSize + offsets[i];
    assert.equal(realOffset % heap.ptrSize, 0, 'not aligned code offset');
    binding.writeTagged(res, offsets[i], off);

    // Keep cross-references for GC
    this.codeOffs.add(res.slice(realOffset));
  }
