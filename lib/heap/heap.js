...
Heap.prototype.scope = function scope(cb) {
...
Heap.prototype.get = function get(ptr) {
  ...
  } else if (this.checkMap('hashmap', ptr)) {
    return new heap.entities.HashMap(this, ptr);
  } else if (this.checkMap('code', ptr)) {
    // TODO(indutny): support code
    return null;
  } else if (this.checkMap('map', ptr) || this.checkMap('hole', ptr)) {
    return null;
  } else {
    console.log(ptr.slice(0, 8), Object.keys(this.maps).map(function(key) {
      return this.maps[key].scope.storage.slice(this.maps[key].index * 8, this.maps[key].index * 8 + 8);
    }, this));
...
Heap.prototype.getSpace = function getSpace(ptr) {
  if (typeof ptr === 'object' && this.checkMap('code', ptr))
    return 'code';
  else
    return 'data';
};
